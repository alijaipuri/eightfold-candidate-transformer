...
class ATSJSONParser(BaseSourceParser):
    """
    Parser for ATS exported candidate records.
    """
    ...
    def parse(
        ...
        if not path.exists():
            raise SourceParsingError(
                f"ATS file not found: {file_path}"
            )
        ...
        try:
            ...
            logger.info(
                "ATS source parsed successfully."
            )
            ...
            raise SourceParsingError(
                f"Failed to parse ATS JSON: {exc}"
            ) from exc
