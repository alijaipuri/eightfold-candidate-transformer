...
class RecruiterNotesParser(BaseSourceParser):
    """
    Parser for recruiter notes and comments.
    """
    ...
    def parse(
        ...
        if not path.exists():
            raise SourceParsingError(
                f"Recruiter notes file not found: {file_path}"
            )
        ...
        try:
            ...
            logger.info(
                "Recruiter notes parsed successfully."
            )
            ...
            raise SourceParsingError(
                f"Failed to parse recruiter notes: {exc}"
            ) from exc
